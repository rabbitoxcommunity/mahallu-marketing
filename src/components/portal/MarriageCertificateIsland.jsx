...
import { searchMarriageCertificates, fetchMarriageCertificateBlob } from '../../lib/portalApi';
...
const MarriageCertificateIsland = ({ slug }) => {
  ...
  const doSearch = (val) => {
    ...
    debounceRef.current = setTimeout(async () => {
      setSearching(true);
      try {
        const res = await searchMarriageCertificates(slug, val.trim());
        setResults(res.data || []);
      } catch {
        setResults([]);
      } finally {
  ...
  const handleDownload = async (certNo, marriageId) => {
    const id = certNo || marriageId;
    setDownloading(id);
    setError('');
    try {
      const blob = await fetchMarriageCertificateBlob(slug, id);
...
export default MarriageCertificateIsland;
